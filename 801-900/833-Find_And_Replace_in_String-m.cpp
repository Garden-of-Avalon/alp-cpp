...
class Solution {
  public:
    string findReplaceString(string S, vector<int> &indexes,
        ...
        int sz = indexes.size();
        vector<pair<int, int>> Spos_ISTpos(sz);
        // Spos means the index in S
        // ISTpos means the index in indexes, sources, and targets
        for (int i = 0; i < sz; ++i)
            Spos_ISTpos[i] = make_pair(indexes[i], i);
        sort(Spos_ISTpos.begin(), Spos_ISTpos.end());

        auto checkSandSrc = [&](int si, int srci) {
            return S.substr(si, sources[srci].size()) == sources[srci];
        };

        string ret;
        int st = 0;
        for (int i = 0; i < sz; ++i) {
            // add middle chars between sources
            if (Spos_ISTpos[i].first > st) {
                ret += S.substr(st, Spos_ISTpos[i].first - st);
                st = Spos_ISTpos[i].first;
            }

            if (checkSandSrc(Spos_ISTpos[i].first, Spos_ISTpos[i].second)) {
                ret += targets[Spos_ISTpos[i].second];
                st += sources[Spos_ISTpos[i].second].size();
            } else if (i < sz - 1) {
                st = Spos_ISTpos[i + 1].first;
                ret +=
                    S.substr(Spos_ISTpos[i].first, st - Spos_ISTpos[i].first);
            }
        }

        return ret + S.substr(st);
    }
};