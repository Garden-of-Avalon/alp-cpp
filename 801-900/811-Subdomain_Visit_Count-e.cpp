...
class Solution {
    ...
    void parse(const string &cpdomain) {
        auto i = cpdomain.find(' ');
        int cnt = stoi(cpdomain.substr(0, i));

        while (i != string::npos) {
            data[cpdomain.substr(++i)] += cnt;
            i = cpdomain.find('.', i);
        }
    }
  ...
  public:
    vector<string> subdomainVisits(vector<string> &cpdomains) {
        ...
        vector<string> ret;
        for (auto &&d_c : data)
            ret.push_back(to_string(d_c.second) + ' ' + d_c.first);
