...
class Solution {
  public:
    string getHint(string secret, string guess) {
        ...
        unordered_multiset<char> unmatch_secret, unmatch_guess;

        for (auto i = 0; i < secret.size(); ++i)
            if (secret[i] == guess[i])
                ++bulls;
            else {
                unmatch_secret.emplace(secret[i]);
                unmatch_guess.emplace(guess[i]);
            }

        unordered_multiset<char>::iterator it;

        for (auto &&g : unmatch_guess)
            if ((it = unmatch_secret.find(g)) != unmatch_secret.end()) {
                ++cows;
                unmatch_secret.erase(it);
            }
...
class Solution {
  public:
    string getHint(string secret, string guess) {
        ...
        // or use map
        array<int, 10> unmatch_secret = {0}, unmatch_guess = {0};

        for (auto i = 0; i < secret.size(); ++i)
            if (secret[i] == guess[i])
                ++bulls;
            else {
                ++unmatch_secret[secret[i] - '0'];
                ++unmatch_guess[guess[i] - '0'];
            }

        for (int i = 0; i < 10; ++i)
            cows += min(unmatch_secret[i], unmatch_guess[i]);
