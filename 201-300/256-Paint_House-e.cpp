...
class Solution {
  public:
    int minCost(vector<vector<int>> &costs) {
        ...
        // costs if current pick i
        vector<int> curr = {costs[0][0], costs[0][1], costs[0][2]};

        for (int i = 1; i < costs.size(); ++i) {
            vector<int> temp(3);
            temp[0] = costs[i][0] + min(curr[1], curr[2]);
            temp[1] = costs[i][1] + min(curr[0], curr[2]);
            temp[2] = costs[i][2] + min(curr[0], curr[1]);
            swap(curr, temp);
        }
