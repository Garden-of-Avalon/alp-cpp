...
// importance value of employee 1 is 5 + 3 + 3 = 11. Note: One employee has at
// most one direct leader and may have several subordinates. The maximum number
// of employees won't exceed 2000.
  ...
  public:
    int getImportance(vector<Employee *> employees, int id) {
        ...
        for (auto &&p : employees)
            id_emp.emplace(p->id, p);

        // -> int is necessary
        auto cal = [&](auto &&self, int id) -> int {
            int sum = 0;
            for (auto &&subid : id_emp[id]->subordinates)
                sum += self(self, subid);
            return sum + id_emp[id]->importance;
        };
