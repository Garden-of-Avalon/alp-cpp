...
class Solution {
  public:
    Node *treeToDoublyList(Node *root) {
        ...
        Node *head = root, *tail = root, *L = root->left, *R = root->right;

        if (L) {
            head = treeToDoublyList(L);
            tail = head->left;
        ...
        if (R) {
            Node *r_head = treeToDoublyList(R);
            Node *r_tail = r_head->left;
