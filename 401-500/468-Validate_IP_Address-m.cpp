...
class Solution {
  public:
    string validIPAddress(string IP) {
        regex ipv4("([1-9]\\d{0,2}|0)\\.([1-9]\\d{0,2}|0)\\.([1-9]\\d{0,2}|0)"
                   "\\.([1-9]\\d{0,2}|0)");
        regex ipv6("([0-9a-fA-F]{1,4})(\\:[0-9a-fA-F]{1,4}){7}");

        smatch m;
        if (regex_match(IP, m, ipv4)) {
            // 1st iter is the whole match, check from 2nd
  ...
  public:
    string validIPAddress(string IP) {
        if (IP.find('.') != string::npos) {
            ...
            istream_iterator<string> ipsit(ips);
            vector<string> numholder(ipsit, {});

            if (numholder.size() == 4) {
                regex num("([1-9]\\d{0,2}|0)");
                smatch mat;
                for (auto &n : numholder)
                    if (!regex_match(n, mat, num) || stoi(n) > 255)
                        return "Neither";
                return "IPv4";
            }
        }
            ...
            if (numholder.size() == 8) {
                regex num("([0-9a-fA-F]{1,4})");
                smatch mat;
                for (auto &n : numholder)
                    if (!regex_match(n, mat, num))
                        return "Neither";
                return "IPv6";
            }
        }
