...
// 0-9) and dashes(-). String S is non-empty.
...
class Solution {
  public:
    string licenseKeyFormatting(string S, int K) {
        ...
        // before: 5F3Z-2e-9-w
        // after:  5F3Z2e9w9-w
        //                 ^ return iter of remove()
        S.erase(remove(S.begin(), S.end(), '-'), S.end()); // erase a range

        for_each(S.begin(), S.end(), [](auto &c) { c = toupper(c); });

        string ret;
        int st = S.size() % K;
        ret += S.substr(0, st ? st : K);

        for (st = ret.size(); st < S.size(); st += K)
            (ret += '-') += S.substr(st, K);
