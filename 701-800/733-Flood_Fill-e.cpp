...
// DFS
class Solution {
    void FF(vector<vector<int>> &image, int sr, int sc, const int &newColor,
            const int &sColor) {
        if (sr >= 0 && sr < image.size() && sc >= 0 && sc < image[0].size() &&
            image[sr][sc] == sColor) {
            image[sr][sc] = newColor;
            FF(image, sr, sc - 1, newColor, sColor);
            FF(image, sr, sc + 1, newColor, sColor);
            FF(image, sr - 1, sc, newColor, sColor);
            FF(image, sr + 1, sc, newColor, sColor);
        }
    }
  ...
  public:
    vector<vector<int>> floodFill(vector<vector<int>> &image, int sr, int sc,
        ...
        if (newColor != sColor)
            FF(image, sr, sc, newColor, sColor);
