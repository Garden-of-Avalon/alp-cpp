...
class Solution {
  public:
    int dominantIndex(vector<int> &nums) {
        pair<int, int> lar = make_pair(-1, -1),
                       slar = lar; // second largest

        for (int i = 0; i != nums.size(); ++i)
            if (nums[i] > lar.first) {
                slar = lar;
                lar = make_pair(nums[i], i);
            } else if (nums[i] != lar.first && nums[i] > slar.first)
                slar = make_pair(nums[i], i);

        return slar.second == -1 || lar.first >= slar.first * 2 ? lar.second
                                                                : -1;
    }
};